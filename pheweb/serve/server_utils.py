...
class _ParseVariant:
    ...
    chrom_pos_ref_alt_regex = re.compile(
        chrom_pos_regex.pattern + r"[-_:/ ]([-AaTtCcGg\.]+)[-_:/ ]([-AaTtCcGg\.]+)"
    )

    def parse_variant(self, query, default_chrom_pos=True):
        match = (
            self.chrom_pos_ref_alt_regex.match(query)
            or self.chrom_pos_regex.match(query)
            or self.chrom_regex.match(query)
        )
        ...
        if default_chrom_pos:
            ...
            if len(g) == 1:
                g += (0,)
        if len(g) >= 2:
            g = (g[0], int(g[1])) + tuple([bases.upper() for bases in g[2:]])
...
def get_random_page() -> Optional[str]:
    ...
    if r < 0.4:
        ...
        return url_for(
            ...
            region="{}:{}-{}".format(
                hit["chrom"], hit["pos"] - offset, hit["pos"] + offset
            ),
        )
