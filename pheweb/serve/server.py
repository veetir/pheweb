...
# --- At the top, after your other imports and blueprint setup ---

import re
import json

# --- Association region endpoint is already defined. Now add a new route for gene data.
@bp.route("/api/region/<phenocode>/lz-genes/")
@check_auth
def api_region_genes(phenocode: str):
    """
    This route returns gene data for the region.
    If no gene data is available for the given region, we return an empty list.
    (LocusZoom expects a JSON array; returning an empty array avoids missing-field errors.)
    """
    return json.dumps([]), 200, {'Content-Type': 'application/json'}

# --- Next, if you’re serving the association data via your custom adapter,
# you can modify its behavior by patching its annotation step.
# Since the adapter’s code is front-end JavaScript, we can instead patch our API
# response to ensure that each association record includes a "variant" key.
# Here’s an example of a wrapper around get_pheno_region:

def get_pheno_region_with_variant(phenocode: str, chrom: str, pos_start: int, pos_end: int):
    """
    Wraps the original get_pheno_region() call and injects a "variant" key into each record.
    The "variant" is built from the record's chromosome, position, ref, and alt.
    """
    data = get_pheno_region(phenocode, chrom, pos_start, pos_end)
    # If data is a dict containing an array (for example, under a key like "assoc"), adjust as needed.
    # Here we assume data is a list of records.
    if isinstance(data, dict) and "assoc" in data:
        records = data["assoc"]
    ...
    for record in records:
        # Only add the variant key if it isn’t already present.
        if "variant" not in record:
            try:
                # Expect that the record has keys: 'chr', 'position', 'ref', and 'alt'
                record["variant"] = "{}:{}_ {}/{}".format(
                    record.get("chr", ""),
                    record.get("position", ""),
                    record.get("ref", ""),
                    record.get("alt", "")
                ...
                # In case of any error, set it to an empty string.
                record["variant"] = ""
    # If we modified records inside a dictionary, reassign it.
    if isinstance(data, dict) and "assoc" in data:
        data["assoc"] = records
        return data
    return records

@bp.route("/api/region/<phenocode>/lz-results/")
@check_auth
def api_region(phenocode: str):
    ...
    if not m:
        ...
        data = get_pheno_region(phenocode, chrom, pos_start, pos_end)
        # If the data is a list of association records, add the "variant" key if missing.
        if isinstance(data, list):
            for record in data:
                if "variant" not in record:
                    # Make sure the required fields exist before constructing "variant".
                    if all(k in record for k in ("chr", "position", "ref", "alt")):
                        record["variant"] = f"{record['chr']}:{record['position']}_{record['ref']}/{record['alt']}"
        return jsonify(data)
    

@bp.route("/region/<phenocode>/results/")
@check_auth
def legacy_region_results(phenocode: str):
    """
    This route supports legacy URLs generated by the front end.
    It accepts URLs of the form:
      /region/<phenocode>/results/?filter=chromosome in 'X' and position ge Y and position le Z
    and returns the JSON for that region, just as /api/region/<phenocode>/lz-results/ would.
    """
    filter_param = request.args.get("filter")
    if not isinstance(filter_param, str):
        abort(404)
