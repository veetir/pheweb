    ...
    .then(function(data) {
      ...
      Plotly.newPlot('plotly-gwas-catalog', [traceUKBB, traceEBI], layout)
        .then(function() {
          ...
          var rsidMap = {};
          ukbbCustom.forEach(function(r,i){ if(r.rsid && r.rsid !== 'N/A') rsidMap[r.rsid] = {trace:0, point:i}; });
          ebiCustom.forEach(function(r,i){ if(r.rsid && r.rsid !== 'N/A') rsidMap[r.rsid] = {trace:1, point:i}; });
          ...
          document.addEventListener('variant-hover', function(e){
            var loc = rsidMap[e.detail.rsid];
            if (loc) {
              Plotly.Fx.hover(plotDiv, [{curveNumber:loc.trace, pointNumber:loc.point}]);
            }
          });
