...
_single_filepaths: Dict[str, Callable[[], str]] = {
    ...
    "best-phenos-by-gene-sqlite3": (
        lambda: get_generated_path("best-phenos-by-gene.sqlite3")
    ),
    "best-phenos-by-gene-old-json": (
        lambda: get_generated_path("best-phenos-by-gene.json")
    ),
...
def get_pheno_filepath(kind: str, phenocode: str, *, must_exist: bool = True) -> str:
    if kind not in _pheno_filepaths:
        raise Exception("Unknown kind of filepath: {}".format(repr(kind)))
    filepath: str = _pheno_filepaths[kind](phenocode)
    if must_exist and not os.path.exists(filepath):
        raise PheWebError(
            "Pheno filepath {} of kind {} for phenocode {} was requested but doesn't exist".format(
                filepath, kind, phenocode
            )
        )
    return filepath
...
class _vfr:
    ...
    def _get_variants(self) -> Iterator[Dict[str, Any]]:
        parsers: List[Callable[[str], Any]] = [
            parse_utils.reader_for_field[field] for field in self.fields
        ]
...
class MatrixReader:
    def __init__(self):
        ...
        self._colidxs_for_pheno: Dict[str, Dict[str, int]] = (
            {}
        )  # maps phenocode -> field -> column_index
...
class _mr(_ivfr):
    ...
    def _parse_field(
        self, variant_row: List[str], field: str, phenocode: Optional[str] = None
    ) -> Any:
        colidx = (
            self._colidxs[field]
...
def write_heterogenous_variantfile(
    filepath: str, assocs: List[Dict[str, Any]], use_gzip: bool = True
) -> None:
    """inject all necessary keys into the first association so that the writer will be made correctly"""
    if len(assocs) == 0:
        raise PheWebError(
            "ERROR: tried to write file {!r} but didn't supply any variants"
        )
    assocs[0] = {
        field: assocs[0].get(field, "")
        for field in set(itertools.chain.from_iterable(assocs))
    }
...
def write_json(
    *,
    ...
    with AtomicSaver(
        ...
        json.dump(
            data, f, indent=indent, sort_keys=sort_keys, default=_json_writer_default
        )
...
def _json_writer_default(obj: Any) -> Any:
    ...
    if isinstance(obj, np.float32):
        return float(obj)
    raise TypeError(
        "Object {!r} of type {} is not JSON serializable!".format(
