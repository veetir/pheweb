...
if sys.version_info < (3, 6):
    print(
        "PheWeb requires Python 3.6 or newer.  Use Miniconda or Homebrew or another solution to install a newer Python."
    )
    sys.exit(1)
...
""".split():
    ...
    def f(submodule: str, argv: List[str]) -> None:
        ...
        if not callable(module_run):
            raise Exception(
                "module.run ({!r}) isn't callable for module {!r}".format(
                    module_run, module
                )
            )
        module_run(argv)
