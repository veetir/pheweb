...
def _extract_star(glob_pattern, filepath):
    if "*" not in glob_pattern:
        raise PheWebError(
            "You tried to use --star-is-phenocode, but your pattern {!r} doesn't contain any *s.  If you tried to use a *, maybe wrap your pattern in single-quotes (') to prevent your shell from expanding it."
        )
    ...
    if boltons.iterutils.same(matches):
        return matches[0]
    raise PheWebError(
        "You used --star-is-phenocode with the pattern {!r} which found the file {!r}, but the *s appeared to match {!r}, which are different.".format(
...
def extract_phenocode_from_filepath(phenolist, regex):
    ...
    for pheno in phenolist:
        if "assoc_files" not in pheno:
            raise PheWebError(
                "ERROR: At least one phenotype doesn't have the key 'assoc_files'."
            )
        if not pheno["assoc_files"]:
            raise PheWebError(
                "ERROR: At least one phenotype has an empty 'assoc_files' list."
            )
        phenocodes = []
        for assoc_filepath in pheno["assoc_files"]:
            ...
            if match is None:
                raise PheWebError(
                    "ERROR: The regex {!r} doesn't match the filepath {!r}".format(
                        regex.pattern, assoc_filepath
                    )
                )
            groups = match.groups()
            if len(groups) != 1:
                raise PheWebError(
                    "ERROR: The regex {!r} doesn't capture any groups on the filepath {!r}!  You're using parentheses without backslashes, right?".format(
                        regex.pattern, assoc_filepath
                    )
                )
        ...
        if len(set(phenocodes)) != 1:
            raise PheWebError(
                "ERROR: At least one phenotype gets multiple different phenocodes from its several association filepaths.  Here they are: {!r}".format(
                    list(set(phenocodes))
                )
            )
...
def check_that_columns_are_present(phenolist, columns):
    ...
    for col in columns:
        ...
        if phenos_without_col:
            ...
            error_message += "\n"
    if error_message:
        raise PheWebError(error_message)
...
def check_that_all_phenos_have_same_columns(phenolist):
    all_columns = list(
        boltons.iterutils.unique(col for pheno in phenolist for col in pheno)
    )
    for pheno in phenolist:
        for col in all_columns:
            if col not in pheno:
                raise PheWebError(
                    "ERROR: the column {!r} is in at least one phenotype but not in {!r}".format(
                        col, pheno
                    )
                )
...
def check_that_all_phenotypes_have_assoc_files(phenolist):
    for pheno in phenolist:
        ...
        if any(not isinstance(s, str) for s in pheno["assoc_files"]):
            raise PheWebError(
                "assoc_files contains things other than strings for some phenotypes."
            )
            ...
            if pheno["num_samples"] != total_samples:
                raise PheWebError(
                    "The pheno {} has num_samples={} but num_cases+num_controls={}: {}".format(
                        pheno["phenocode"], pheno["num_samples"], total_samples, pheno
                    )
                )
...
def filter_phenolist(phenolist, filter_func, name_for_debugging=""):
    ...
    print(
        "running filter {}: {} phenos pass, {} phenos fail.".format(
            name_for_debugging, len(passing_phenos), len(failing_phenos)
        )
    )
    if failing_phenos:
    ...
    with read_maybe_gzip(filepath) as f:
        ...
        try:
            ...
            if filepath.endswith(".json"):
                raise PheWebError(
                    "The filepath {!r} ends with '.json' but reading it as json failed.".format(
                        filepath
                    )
                )
...
def _import_phenolist_csv(f, has_header):
    ...
    try:
        ...
        raise PheWebError(
            "Sniffing csv format failed.  Check that your csv file is well-formed.  If it is, try delimiting with tabs or semicolons."
        ) from exc
    if dialect.delimiter in string.ascii_letters or dialect.delimiter in string.digits:
        raise PheWebError(
            "Our csv sniffer decided that {!r} looks like the most likely delimiter in your csv file, but that's crazy.".format(
                dialect.delimiter
            )
        )
    f.seek(0)
    ...
    if has_header:
        ...
        if any(fieldname is None or fieldname == "" for fieldname in fieldnames):
            if has_header == "augment":
                fieldnames = [
                    i if fieldname is None else fieldname
        ...
        for k in pheno:
            if isinstance(pheno[k], str) and pheno[k].startswith("json:"):
                ...
                try:
                    ...
                    raise PheWebError(
                        "The input file contained an invalid field marked to be interpreted as json: {!r}".format(
                            pheno[k]
                        )
                    ) from exc
    return phenolist
...
def split_values_on_pipes(phenolist):
    ...
    str_keys = [
        key
        for key in all_keys
        if all(isinstance(pheno.get(key, None), str) for pheno in phenolist)
    ]
    pipe_keys = [
        key for key in str_keys if any("|" in pheno.get(key, "") for pheno in phenolist)
    ]
    if pipe_keys:
        print(
            "Here are the keys that are going to be split into lists (ie, all values are strings and at least one contains '|'):"
        )
    ...
    for pheno in phenolist:
        if "assoc_files" in pheno and not isinstance(pheno["assoc_files"], list):
            if not isinstance(pheno["assoc_files"], str):
                raise PheWebError(
                    "assoc_files is of unsupported type({!r}). value: {!r}".format(
                        type(pheno["assoc_files"]), pheno["assoc_files"]
                    )
                )
...
def numify_numeric_cols(phenolist):
    ...
    def floaty(value):
        return isinstance(value, str) and float_regex.match(value)

    def inty(value):
        return isinstance(value, str) and int_regex.match(value)
    ...
    for pheno in phenolist:
        for k in pheno:
            if isinstance(pheno[k], (int, float)):
                ...
                isinstance(pheno[k], list)
                and len(pheno[k]) > 0
                and all(isinstance(v, str) for v in pheno[k])
                and all("|" not in v for v in pheno[k])
            ):
...
def rename_column(phenolist, old_name, new_name):
    for pheno in phenolist:
        if new_name in pheno:
            raise PheWebError(
                "ERROR: You're renaming the column {!r} to {!r}, but {!r} already exists in the phenotype {!r}.".format(
                    old_name, new_name, new_name, pheno
                )
            )
        if old_name not in pheno:
            raise PheWebError(
                "ERROR: You're renaming the column {!r} to {!r}, but {!r} doesn't exist in the phenotype {!r}.".format(
                    old_name, new_name, old_name, pheno
                )
            )
...
def merge_in_info(phenolist, more_info_rows, allow_missing_fields=False):
    ...
    keys_to_add = set(itertools.chain.from_iterable(more_info_rows)) - set(
        itertools.chain.from_iterable(phenolist)
    )
    ...
    for pheno in phenolist:
        ...
        if row is None:
            raise PheWebError(
                "ERROR: there's no row in your info-to-merge file with the phenocode {!r}".format(
                    pheno["phenocode"]
                )
            )
        for key in keys_to_add:
            if not allow_missing_fields and key not in row:
                raise PheWebError(
                    "Trying to pull in the keys {}, but the row {} doesn't have them".format(
                        keys_to_add, row
                    )
                )
            if key in row:
...
def unique_phenocode(phenolist, new_column_name):
    ...
    if not all("phenocode" in pheno for pheno in phenolist):
        raise PheWebError(
            "At least one pheno doesn't have a 'phenocode', so you can't run unique-phenocode"
        )
    if not boltons.iterutils.same(pheno.keys() for pheno in phenolist):
        raise PheWebError(
            "Not all phenotypes have the same columns.  That probably not a problem, but I haven't thought through the implications yet."
        )
    phenocode_groups = boltons.iterutils.bucketize(
        phenolist, lambda p: p["phenocode"]
    ).values()
    ...
    print(
        "NOTE: the columns {!r} sometimes have multiple values for the same phenocode so we'll combine them.".format(
            list(columns_to_listify)
        )
    )
    if len(columns_to_listify) == 0:
        ...
        for phenocode_group in phenocode_groups:
            ...
            for key in phenocode_group[0]:
                if key not in columns_to_listify:
                    ...
                    assert all(
                        row[key] == phenocode_group[0][key] for row in phenocode_group
                    )
                else:
                    if not boltons.iterutils.same(
                        type(row[key]) for row in phenocode_group
                    ):
                        raise PheWebError(
                            "ERROR: there are multiple types in the column {!r}, so I don't want to make it a list".format(
                                key
                            )
                        )
                    if isinstance(phenocode_group[0][key], list):
                        # TODO: assert that the elements of the lists are all the same type
                        items_to_add = itertools.chain.from_iterable(
                            row[key] for row in phenocode_group
                        )
                    else:
                        if not isinstance(
                            phenocode_group[0][key], (str, int, float, dict)
                        ):
                            raise PheWebError(
                                "Where did you even get the type {!r}?".format(
                                    type(phenocode_group[0][key])
                                )
                            )
                        items_to_add = (row[key] for row in phenocode_group)
                    new_pheno[key] = list(
                        set(_get_hashable(item) for item in items_to_add)
                    )
        ...
        for phenocode_group in phenocode_groups:
            ...
            for row in phenocode_group:
                new_pheno[new_column_name].append(
                    {key: row[key] for key in columns_to_listify}
                )
            new_pheno[new_column_name] = list(
                set(_get_hashable(d) for d in new_pheno[new_column_name])
            )
            new_phenolist.append(new_pheno)
        return new_phenolist
...
def save_phenolist(phenolist, filepath=None):
    ...
    print(
        "NOTE: wrote {} phenotypes to {!r} with columns {!r}".format(
            len(phenolist), filepath, all_columns
        )
    )


def backup_phenolist(filepath):
    backup_filepath = "{}-{}".format(
        datetime.datetime.isoformat(datetime.datetime.now()), os.path.basename(filepath)
    )
...
def run(argv):
    ...
    )  # don't throw a messy exception when running `pheweb phenolist ... | head`
    ...
    def f(args):
        ...
        filepath = args.filepath or default_phenolist_filepath
        phenolist = get_phenolist_with_globs(
            args.patterns, star_is_phenocode=args.star_is_phenocode
        )
        if args.simple_phenocode:
            pattern = (
                r".*/(?:(?:epacts|pheno)[\.-]?)?"
                + r"([^/]+?)"
                + r"(?:\.chr(?:[1-9][0-9]?|X|Y|MT?)|\.epacts|\.gz|\.tsv|\.txt|\.csv)*$"
            )
    ...
    p.add_argument(
        "--simple-phenocode",
        dest="simple_phenocode",
        action="store_true",
        help="Extract a simple phenocode from the end of each filepath",
    )
    p.add_argument(
        "--star-is-phenocode",
        dest="star_is_phenocode",
        action="store_true",
    ...
    p = subparsers.add_parser(
        "extract-phenocode-from-filepath",
        help="use a regex to extract phenocodes from association filepaths",
    )
    p.add_argument(
        "pattern",
        nargs="?",
        help="a perl-compatible regex pattern with one capture group",
    )
    p.add_argument(
        "-f",
        dest="filepath",
        help="pheno-list filepath, used for both input and output (default: {!r})".format(
    ...
    p.add_argument(
        ...
        dest="simple",
        action="store_true",
        help="just take whatever's between the last / and the first .",
    )
    ...
    p = subparsers.add_parser(
        "unique-phenocode", help="if multiple rows have the same phenocode, merge them"
    )
    p.add_argument(
    ...
    def f(args):
        ...
        check_that_columns_are_present(
            phenolist, ["phenocode", "assoc_files"] + args.required_columns
        )
    ...
    p = subparsers.add_parser(
        "verify",
        help="check that pheno-list is well-formed and could plausibly be used to make a pheweb",
    )
    p.add_argument(
        "-f",
        dest="filepath",
        help="pheno-list filepath to check (default: {!r})".format(
            default_phenolist_filepath
        ),
    )
    p.add_argument(
        ...
        nargs="+",
        default=[],
        help="a list of column names that must be included in all phenotypes",
    )
    ...
    def f(args, phenolist):
        ...
        if args.minimum_num_controls is not None:
            phenolist = filter_phenolist(
                phenolist,
                lambda p: p.get("num_controls", float("inf"))
                >= args.minimum_num_controls,
                "minimum_num_controls",
            )
        if args.minimum_num_samples is not None:
            phenolist = filter_phenolist(
                phenolist,
                lambda p: p.get("num_samples", float("inf"))
                >= args.minimum_num_samples,
                "minimum_num_samples",
            )
        return phenolist

    p = subparsers.add_parser(
        "filter-phenotypes", help="filter the phenotypes using various rules"
    )
    p.add_argument(
        "-f",
        dest="filepath",
    ...
    p.add_argument(
        "--minimum-num-cases",
        dest="minimum_num_cases",
        type=int,
        help="remove any phenotypes with fewer than this number of cases",
    )
    p.add_argument(
        "--minimum-num-controls",
        dest="minimum_num_controls",
        type=int,
        help="remove any phenotypes with fewer than this number of controls",
    )
    p.add_argument(
        "--minimum-num-samples",
        dest="minimum_num_samples",
        type=int,
        help="remove any phenotypes with fewer than this number of samples",
    )
    ...
    p = subparsers.add_parser(
        "hide-small-numbers-of-samples",
        help="if a phenotype has too few samples, cases, or controls, don't show the number",
    )
    p.add_argument(
        "-f",
        dest="filepath",
    ...
    p.add_argument(
        "--minimum-visible-number",
        dest="minimum_visible_number",
        type=int,
        help="the minimum number of samples, cases, or controls that will be visible",
    )
    ...
    p = subparsers.add_parser(
        "read-info-from-association-files",
        help="read per-pheno fields from association files",
    )
    p.add_argument(
        "-f",
        dest="filepath",
    ...
    p = subparsers.add_parser(
        "import-phenolist",
        help="read a csv or tsv, optionally gzipped, and produce a json file",
    )
    ...
    p.add_argument(
        ...
        dest="no_header",
        action="store_true",
        help="whether input_filepath has no header, in which case columns will just be numbered",
    )
    p.add_argument(
        "--never-delimit-lists-with-pipe",
        dest="delimit_lists_with_pipe",
        action="store_false",
        help="whether to split any fields that contain pipes into lists",
    )
    ...
    p = subparsers.add_parser(
        "print-as-csv",
        help="Produce a csv file that could be read in by import-phenolist",
    )
    p.add_argument(
        "-f",
        dest="filepath",
    ...
    p.add_argument(
        "columns_to_keep",
        nargs="+",
        help="a list of column names to keep in phenotypes -- the rest will be deleted",
    )
    ...
    p.add_argument(
        "renames",
        nargs="+",
        help="columns to rename, in pairs, like this: <oldname> <newname> <oldname> <newname>...",
    )
    ...
    def f(args, phenolist):
        ...
        phenolist = merge_in_info(
            phenolist, more_info_file, allow_missing_fields=args.allow_missing_fields
        )
        return phenolist
    ...
    p.add_argument(
        ...
        action="store_true",
        help="Allow some fields to be missing on some phenotypes (unlike the default, which requires all phenotypes to have the same fields)",
    )
    p.add_argument(
        "file_with_more_info",
        help="a pheno-list file with more information to add to the main pheno-list file",
    )
