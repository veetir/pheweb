...
def get_genenamesorg_ensg_aliases_map(
    ensgs_to_consider: Iterable[str],
) -> Dict[str, List[str]]:
    ...
    for row in json.loads(data)["response"]["docs"]:
        try:
            if (
                not row.get("ensembl_gene_id", None)
                or row["ensembl_gene_id"] not in ensgs_to_consider
            ):
                continue
            ...
            aliases = [
                alias for alias in aliases if re.match(r"^[-\._a-zA-Z0-9]+$", alias)
            ]
...
def get_gene_aliases() -> Dict[str, str]:
    ...
    genes = [
        {"canonical": canonical, "ensg": ensg}
        for _, _, _, canonical, ensg in get_gene_tuples_with_ensg()
    ]
    assert len({g["ensg"] for g in genes}) == len(genes)
    assert len({g["canonical"] for g in genes}) == len(genes)
    for g in genes:
        assert re.match(
            r"^ENSGR?[0-9]+(?:\.[0-9]+(?:_[0-9]+)?(?:_PAR_[XY])?)?$", g["ensg"]
        ), g
    ...
    return {
        alias: ",".join(canonicals) for alias, canonicals in alias_to_canonicals.items()
    }
