...
def run(argv: List[str]) -> None:
    parser = argparse.ArgumentParser(
        description="annotate each phenotype by pulling in information from the combined sites file"
    )
    parser.add_argument(
        "--phenos",
...
def convert(pheno: Dict[str, Any]) -> None:
    ...
    with VariantFileReader(sites_filepath) as sites_reader, VariantFileReader(
        ...
        try:
            ...
            raise PheWebError(
                "It appears that the phenotype {!r} has no variants.".format(
                    pheno["phenocode"]
                )
            )
        try:
            ...
            raise PheWebError(
                "It appears that your sites file ({!r}) has no variants.".format(
                    sites_filepath
                )
            )
        while True:
            ...
            if cmp == 1:  # pheno variant is ahead, so advance sites variant
                try:
                    ...
                    raise PheWebError(
                        "The sites file ({}) ran out of variants while {} still had {}".format(
                            sites_filepath, parsed_filepath, pheno_variant
                        )
                    )
            elif cmp == 2:  # sites variant is ahead, so something went wrong.
                raise PheWebError(
                    "The sites file ({}) is missing a variant that's present in {}: {}.".format(
                        sites_filepath, parsed_filepath, pheno_variant
                    )
                )
