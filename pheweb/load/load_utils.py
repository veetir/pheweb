...
def get_maf(variant: Dict[str, Any], pheno: Dict[str, Any]) -> Optional[float]:
    ...
    if len(mafs) == 0:
        ...
        if any(maf > 0.5 for maf in mafs):
            raise PheWebError(
                "Error: the variant {} in pheno {} has at least one way of computing maf that is > 0.5 ({})".format(
                    variant, pheno, mafs
                )
            )
        ...
        if not isinstance(maf_sigfigs, int):
            raise Exception()
...
def exception_printer(f):
    ...
    def f2(*args, **kwargs):
        try:
            ...
            time.sleep(
                2 * random.random()
            )  # hopefully avoid interleaved printing (when using multiprocessing)
            traceback.print_exc()
            ...
            print(strexc)
            if args:
                print("args were: {!r}".format(args))
            if kwargs:
                print("kwargs were: {!r}".format(args))
            print("")
            raise
        return rv
    ...
    try:
        with open(os.devnull) as devnull:
            ...
            data_bytes = subprocess.check_output(
                ["bash", "-c", script], stderr=subprocess.STDOUT, stdin=devnull
            )
        status = 0
...
class Parallelizer:
            ...
            while True:
                try:
                    ...
                    if not any(p.is_alive() for p in procs):
                        raise PheWebError(
                            "No living children remain and retq is empty, but n_procs={} and taskq.get_nowait()={!r}".format(
                                n_procs, taskq.get_nowait()
                            )
                        )
                    continue
                ...
                if ret["type"] == "result":
                    ...
                    for p in procs:
                        p.is_alive()  # This cleans up zombies
                    if n_procs == 0:
                        self._update_progressbar(
                            progressbar, n_tasks_complete, n_procs, len(tasks)
                        )
                        for p in procs:
                            ...
                            if p.exitcode != 0:
                                raise PheWebError(
                                    "A child process exited with status {}".format(
                                        p.exitcode
                                    )
                                )
                        return
        ...
        if n_procs == 0 and num_tasks == n_tasks_complete:
            ...
            progressbar.set_message(
                ...
                "Completed {:4} tasks in {} ({} in progress, {} queued)".format(
                    ...
                    n_procs,
                    num_tasks - n_tasks_complete - n_procs,
                )
            )
    ...
    def _make_multiple_tasks_doer(do_single_task):
        ...
        return functools.partial(
            Parallelizer._partialable_multiple_tasks_doer, do_single_task
        )
    ...
    def _partialable_multiple_tasks_doer(do_single_task, taskq, retq, parent_overrides):
        if conf.overrides and conf.overrides != parent_overrides:
            err = "Tried to pass parent_overrides {!r} down to child process that already had overrides {!r}".format(
                parent_overrides, conf.overrides
            )
            retq.put(
                {
                    "type": "exception",
        ...
        for task in iter(taskq.get, {"exit": True}):
            try:
                ...
                for ret in (
                    x if isinstance(x, GeneratorType) else [x]
                ):  # if it returns None (rather than a generator), assume it has no results
                    retq.put(
                        {
                            "type": "result",
                            "task": task,
    ...
    def run_on_each_pheno(
        ...
        *,
        ...
        tasks = [
            pheno
            for pheno in phenos
            if self.should_process_pheno(
                pheno, get_input_filepaths, get_output_filepaths
            )
        ]
        if not tasks:
            print(
        ...
        if len(phenos) == len(tasks):
            ...
            print(
                "Processing {} phenos ({} already done)".format(
    ...
    def should_process_pheno(self, pheno, get_input_filepaths, get_output_filepaths):
        ...
        for fp in input_filepaths:
            if not os.path.exists(fp):
                raise PheWebError(
                    "Cannot make {} because {} does not exist".format(
...
def parallelize_per_pheno(
    get_input_filepaths, get_output_filepaths, convert, *, cmd=None, phenos=None
):
    return PerPhenoParallelizer().run_on_each_pheno(
        get_input_filepaths, get_output_filepaths, convert, cmd=cmd, phenos=phenos
    )
