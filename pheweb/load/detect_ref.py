...
class Build:
    ...
    def __repr__(self):
        return "<Build hg_name={} grch_name={}>".format(self.hg_name, self.grch_name)
    ...
    def _download_chrom_file(self, url, filepath, verbose=True):
        ...
        try:
            ...
            tr -d "\n" > '{tmp_filepath}'
            """.format(
                    download_filepath=download_filepath, tmp_filepath=tmp_filepath
                )
            )
...
def detect_build(build_scores, match_threshold=1):
    ...
    for build, score in build_scores.items():
        if score["either"] >= match_threshold:
            if score["a1"] >= match_threshold:
                matching_builds.append((build, "a1"))
            elif score["a2"] >= match_threshold:
                matching_builds.append((build, "a2"))
            else:
                matching_builds.append((build, "either"))
...
def get_build_scores(variant_iterator, builds=None):
    ...
    """
    if builds is None:
        builds = get_default_builds()
    with contextlib.ExitStack() as es:
        for build in builds:
            es.callback(build.close)  # close all the builds' files when we finish.
        assert all(isinstance(build, Build) for build in builds)
        match_counts = OrderedDict(
            (build, {"a1": 0, "a2": 0, "either": 0}) for build in builds
        )
        for num_variants, (chrom, pos, a1, a2) in enumerate(variant_iterator, start=1):
            for build in builds:
                ...
                if a1_matches:
                    match_counts[build]["a1"] += 1
                if a2_matches:
                    match_counts[build]["a2"] += 1
                if a1_matches or a2_matches:
                    match_counts[build]["either"] += 1
...
def progressbar_handle_variants(variant_iterator, builds=None):
    def fmt_match_percent(fraction):
        return "all" if fraction == 1 else "{:2.0f}%".format(100 * fraction)
    ...
    with contextlib.ExitStack() as es:
        ...
        try:
            with ProgressBar() as progressbar:
                for chrom, pos, a1, a2 in variant_iterator:
                        ...
                        " ".join(
                            "{}[{}]".format(
                                ...
                                " ".join(
                                    "{}:{}".format(
                                        allele,
                                        fmt_match_percent(num_match / num_variants),
                                    )
                    ...
                    if not any(
                        d["either"] == num_variants for d in match_counts.values()
                    ):
        ...
        if num_variants:
            ...
            for build, counts in match_counts.items():
                print(
                    build,
                    "   1st allele column matches ref {:3.0f}% (at {:4}/{} variants)".format(
                        counts["a1"] / num_variants * 100, counts["a1"], num_variants
                    ),
                )
                print(
                    build,
                    "   2nd allele column matches ref {:3.0f}% (at {:4}/{} variants)".format(
                        counts["a2"] / num_variants * 100, counts["a2"], num_variants
                    ),
                )
                print(
                    build,
                    "either allele column matches ref {:3.0f}% (at {:4}/{} variants)".format(
                        counts["either"] / num_variants * 100,
                        counts["either"],
                        num_variants,
                    ),
                )
...
def make_variant_iterator(
    filepath_or_file_or_iterable,
    chrom_pos_a1_a2_cols=(0, 1, 2, 3),
    comment_char="#",
    num_header_lines=0,
    limit_num_variants=None,
):
    """
    ...
    if isinstance(filepath_or_file_or_iterable, str):
        with read_maybe_gzip(filepath_or_file_or_iterable) as f:
            yield from make_variant_iterator(
                f,
                chrom_pos_a1_a2_cols,
                comment_char,
                num_header_lines,
                limit_num_variants,
            )
        ...
        for i, line in enumerate(filepath_or_file_or_iterable):
            line = line.rstrip("\n")
            if i < num_header_lines or comment_char and line.startswith(comment_char):
                continue
            if limit_num_variants and i >= limit_num_variants + num_header_lines:
                break
            parts = line.split("\t") if line.count("\t") >= 3 else line.split()
            if len(parts) < 4:
                raise PheWebError(
                    "There should be 4 tab-delimited items (chromosome, position, allel1, allele2) but there are only {} on the line {!r}".format(
                        len(parts), line
                    )
                )
            yield (
                parse_chrom(parts[chrom_col].strip()),
                parse_pos(parts[pos_col].strip()),
                parts[a1_col].strip(),
                parts[a2_col].strip(),
            )
...
def parse_build(build_string):
    for b in get_default_builds():
        if build_string.lower() in (b.hg_name.lower(), b.grch_name.lower()):
            return b
    raise PheWebError(
        "unknown build {!r}, try one of {}".format(
            build_string, [(b.hg_name, b.grch_name) for b in get_default_builds()]
        )
    )
...
def parse_chrom(chrom):
    if chrom.startswith("chr"):
        chrom = chrom[3:]
    if chrom == "MT":
        chrom = "M"  # UCSC says "chrM"
    if chrom not in Build._allowed_chroms:
        raise PheWebError(
            "unknown chromosome {!r} (accepted chromosomes are {} with optional prefix 'chr')".format(
                chrom, Build._allowed_chroms + ["MT"]
            )
        )
    return chrom
...
def parse_pos(pos_string):
    try:
        ...
        raise PheWebError("position {!r} is not an integer".format(pos_string))
...
"""
    ...
    if not argv or any(arg in {"-h", "--help", "help"} for arg in argv):
        ...
        print(
            "{} {}:{:,} {}".format(
                build.hg_name, chrom, pos, "not found" if base is None else base
            )
        )
        ...
        variant_iterator = make_variant_iterator(
            filepath,
            (
                0,
                1,
