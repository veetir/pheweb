...
def download_rsids_for_build(hg_build_number: int) -> None:
    ...
    rsids_filepath = get_filepath(
        "rsids-hg{}".format(hg_build_number), must_exist=False
    )
    ...
    if not os.path.exists(rsids_filepath):
        ...
        if not os.path.exists(raw_dbsnp_filepath):
            ...
            if hg_build_number == 19:
                url = "https://ftp.ncbi.nih.gov/snp/redesign/archive/b{}/VCF/GCF_000001405.25.gz".format(
                    dbsnp_version
                )
        ...
        """.format(
                raw_dbsnp_filepath=raw_dbsnp_filepath,
                rsids_tmp_filepath=rsids_tmp_filepath,
            )
        )
