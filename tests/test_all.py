...
def test_all(tmpdir, capsys):
    ...
    cl_run(
        ...
        + [
            ...
            "import-phenolist",
            "-f",
            "{}/pheno-list-categories.json".format(data_dir),
            "{}/categories.csv".format(input_dir),
        ]
    )
    cl_run(
        conf
    ...
    from pheweb.serve.server import (
        app,
    )  # TODO: this relies on data_dir being set earlier, but shouldn't.
    ...
    with app.test_client() as client:
        ...
        assert (
            client.get(
                "/api/region/snowstorm/lz-results/?filter=chromosome%20in%20%20%278%27%20and%20position%20ge%20976279%20and%20position%20le%201276279"
            ).status_code
            == 200
        )
        assert (
            client.get("/region/snowstorm/gene/DNAH14?include=1-225494097").status_code
            == 200
        )
