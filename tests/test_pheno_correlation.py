...
CORREL_FILE = os.path.join(
    os.path.dirname(__file__), "input_files/correlations/rg-pipeline-output.txt"
)
PHENOLIST = os.path.join(
    os.path.dirname(__file__), "input_files/correlations/pheno-list.json"
)
...
def test_new_column_contains_trait2_descriptions(annotated_sample):
    ...
    assert (
        expected == labels
    ), "Labels correspond to trait 2: phenostring if possible, else phenocode"
...
def test_unknown_phenocodes_get_dropped_from_annotated_file(
    sample_data, annotated_sample
):
    ...
    c1_trait2_column = [
        line.split("\t")[1] for line in c1 if not line.startswith("Trait1")
    ]
    assert "031" in c1_trait2_column, "Input file contains phenocode 031"

    c2_trait2_column = [
        line.split("\t")[1] for line in c2 if not line.startswith("Trait1")
    ]
    assert (
        "031" not in c2_trait2_column
    ), "Annotated file omits phenocode 031 which is missing in pheno-list.json"
