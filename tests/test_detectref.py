def test_detect_ref():
    ...
    filepath = os.path.join(
        ...
        "input_files/",
        "assoc-files",
        "has-fields-ac-af-maf.txt",
    )  # has only chr10
    variant_iterator = pheweb.load.detect_ref.make_variant_iterator(
        filepath, num_header_lines=1
    )
    ...
    for build, score in build_scores.items():
        ...
        for a, frac in score.items():
            assert 0 <= frac <= 1
        assert score["a1"] + score["a2"] >= score["either"]
    matching_build, matching_allele_col = pheweb.load.detect_ref.detect_build(
        build_scores
    )
