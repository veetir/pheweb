...
def sites_reader():
    with VariantFileReader(sites_filepath) as vfr:
        ...
        assert sorted(first_variant.keys()) == sorted(
...
def vep_reader():
    with read_maybe_gzip(vep_filepath) as sites_f:
        reader = csv.DictReader(
            (line.lstrip("#") for line in sites_f if not line.startswith("##")),
            delimiter="\t",
        )
...
with gzip.open(out_filepath, "wt") as out_f:
    writer = csv.DictWriter(
        out_f,
        "chrom pos ref alt rsids nearest_genes consequence".split(),
        delimiter="\t",
    )
    writer.writeheader()

    for site_v, vep_v in itertools.zip_longest(
        sites_reader(), vep_reader(), fillvalue={}
    ):
        # sites_filepath and vep_filepath must have a perfect one-to-one match!
        assert all(site_v[k] == vep_v[k] for k in "chrom pos ref alt".split()), (
            site_v,
            vep_v,
        )
