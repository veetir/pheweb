...
def format_float(x):
    ...
    if re.match(r"^[0-9]e-1$", rv):
        return ".{}".format(rv[0])
    if re.match(r"^[0-9]e-2$", rv):
        return ".0{}".format(rv[0])
    return rv
    ...
    def writerow(self, dct):
        if not hasattr(self, "writer"):
            self.writer = csv.DictWriter(
                self.f, fieldnames=list(dct.keys()), delimiter="\t", lineterminator="\n"
            )
...
variants = requests.get(
    "http://pheweb.sph.umich.edu/api/manhattan/pheno/601.json"
).json()["unbinned_variants"]
    ...
    with TSVWriter("input_files/assoc-files/{}.txt".format(pheno_name)) as writer:
        ...
        for v in variants:
            if (
                v["chrom"] == "1"
                and v["pos"] == 869334
                or random.random() < 100 / len(variants)
            ):
                ...
                ac = random.randrange(
                    0, num_chromosomes + 1
                )  # allow MAF=0 b/c I'm sure somebody will.
                af = ac / num_chromosomes
                if use_maf:
                    d["maf"] = format_float(min(af, 1 - af))
                if use_af:
                    d["af"] = format_float(af)
                if use_ac:
                    d["ac"] = ac
                if use_ns:
                    d["ns"] = ns
