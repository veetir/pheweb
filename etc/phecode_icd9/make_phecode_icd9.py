...
icd9s_2 = list(
    open(
        "ICD-9-CM-v32-master-descriptions/CMS32_DESC_LONG_DX.txt", encoding="ISO-8859-1"
    )
)
string_for_icd9_2 = {}
for x in icd9s_2:
...
def cmp(*xs):
    ...
    for n in range(1, 1 + len(xs)):
        for c in itertools.combinations(range(len(xs)), n):
            ...
            print(
                len(
                    set.intersection(*[set(xs[i]) for i in c]).difference(
                        *[xs[i] for i in comp]
                    )
                ),
                end=" ",
...
for phecode in info_for_phecode:
    ...
    for icd9 in icd9s_for_phecode[phecode]:
        ...
        desc = (
            string_for_icd9_3.get(icd9, False)
